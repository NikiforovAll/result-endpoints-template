...
// ref: https://github.com/dotnet/aspnetcore/issues/44988
/// <summary>
...
public class HttpResultsOperationFilter : IOperationFilter
{
    ...
    public HttpResultsOperationFilter(IOptions<MvcOptions> mvc)
    {
        _contentTypes = new Lazy<string[]>(() =>
        {
            ...
            {
                var jsonApplicationType = mvc.Value.FormatterMappings.GetMediaTypeMappingForFormat(
                    "json"
                );
                if (jsonApplicationType != null)
                    apiResponseTypes.Add(jsonApplicationType);
                var xmlApplicationType = mvc.Value.FormatterMappings.GetMediaTypeMappingForFormat(
                    "xml"
                );
    ...
    void IOperationFilter.Apply(OpenApiOperation operation, OperationFilterContext context)
    {
        ...
        else if (actionReturnType == typeof(UnauthorizedHttpResult))
        {
            ...
            operation.Responses.Add(
                "401",
                new OpenApiResponse { Description = ReasonPhrases.GetReasonPhrase(401) }
            );
        }
    }
    ...
    private static string? GetResponseDescription(string statusCode) =>
        ResponseDescriptionMap.FirstOrDefault(entry => Regex.IsMatch(statusCode, entry.Key)).Value;
